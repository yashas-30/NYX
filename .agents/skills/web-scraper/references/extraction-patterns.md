...
function extractAllTables() {
  ...
  return Array.from(tables).map((table, idx) => {
    const caption =
      ...
      `Table ${idx + 1}`;
    const headers = Array.from(table.querySelectorAll('thead th, tr:first-child th')).map((el) =>
      el.textContent.trim()
    );
...
// Example usage:
JSON.stringify(
  extractList('.results', '.result-item', {
    title: '.result-title',
...
function extractJsonLd(targetType) {
  ...
  const allData = Array.from(scripts)
    .map((s) => {
      try {
  ...
  if (targetType) {
    return flat.filter(
      (d) =>
        d['@type'] === targetType || (Array.isArray(d['@type']) && d['@type'].includes(targetType))
    );
  }
  return flat;
}
...
function extractMeta() {
  ...
  meta.canonical = document.querySelector('link[rel="canonical"]')?.getAttribute('href');
  return meta;
}
...
function extractPricingPlans() {
  ...
  return Array.from(cards).map((card) => ({
    name:
      card.querySelector('[class*="name"], [class*="title"], h2, h3')?.textContent?.trim() || null,
    price: card.querySelector('[class*="price"], [class*="amount"]')?.textContent?.trim() || null,
    period:
      card.querySelector('[class*="period"], [class*="billing"]')?.textContent?.trim() || null,
    features: Array.from(card.querySelectorAll('[class*="feature"] li, ul li')).map((li) =>
      li.textContent.trim()
    ),
...
function extractFAQ() {
  ...
  return Array.from(items).map((item) => ({
    question:
      item
        .querySelector('[class*="question"], [class*="header"], [class*="title"], h3, h4')
        ?.textContent?.trim() || null,
    answer:
      item
        .querySelector('[class*="answer"], [class*="body"], [class*="content"], p')
        ?.textContent?.trim() || null,
  }));
}
