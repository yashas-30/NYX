...
**Best for:** conceptual and logical data models, API resource relationships, domain models — anything where the story is _entities and cardinality_.

**Not for the physical schema.** ER is entity-level: relationship lines join _boxes_ and carry cardinality at each end. When the point is real tables with SQL types, constraint chips, indexes, and foreign keys that anchor **column to column**, use [`type-db-schema.md`](type-db-schema.md) instead.
