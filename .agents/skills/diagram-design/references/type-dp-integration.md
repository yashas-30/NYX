...
Use when the question is _"what surfaces does this platform expose, and over what wire?"_ rather than _"how does data move through phases?"_.
...
sources: # left column, 0..6 nodes
  - {
      name: 'Databases',
      type: 'db',
      subtitle: 'SQL · MariaDB',
      connects_to:
        [{ to: 'NiFi', label: 'JDBC' }, { to: 'Trino', label: 'FEDERATE', style: 'federated' }],
    }
  - {
      name: 'SFTP drops',
      type: 'sftp',
      subtitle: 'scheduled pulls',
      connects_to: [{ to: 'NiFi', label: 'SFTP' }],
    }
  - {
      name: 'Email',
      type: 'mail',
      subtitle: 'IMAP attachments',
...
platform:
  ...
  rows: # ordered top→bottom; each is bar or row
    - {
        ...
        name: 'Trino',
        icon: trino,
        subtitle: 'federated query · push-down',
        role: 'SQL',
        focal: true,
      }
    - {
        ...
        nodes:
          [
            { name: 'Apache NiFi', icon: nifi, role: 'INGEST', subtitle: 'flow-based ETL' },
            {
              ...
              icon: minio,
              role: 'STORE',
              subtitle: 'S3 object store · medallion',
              focal: true,
            },
            {
              name: 'JupyterLab',
    ...
    - {
        ...
        name: 'Apache Airflow',
        icon: airflow,
        subtitle: 'scheduler · DAG triggers · backfill',
        role: 'DAG',
      }
...
consumers: # right column, 0..6 nodes
  ...
  - {
      name: 'Public website',
      type: 'globe',
      subtitle: 'NatStat portal',
      connects_from: [{ from: 'Trino', label: 'HTTPS' }],
    }
  - {
      name: 'API gateway',
      type: 'api',
      subtitle: '3rd-party / OAuth2',
      connects_from: [{ from: 'Trino', label: 'REST' }],
    }

footer: # 0..N cross-cutting bars stacked below zone (full-canvas width)
  - { name: 'Active Directory', icon: key, subtitle: 'LDAP · SSO · group RBAC', color: '#b85450' } # tinted red to flag the security concern
...
**Active Directory** (or Keycloak, IAM, OPA, any cross-cutting identity / policy / secrets store) authenticates _every_ component in the platform. Wiring it to one specific tool would understate the trust scope. Connect it instead with a single arrow to the bottom edge of the platform zone, labeled `AUTH` (§3.3).
...
That's **14–20 nodes**. The complexity is the point — the diagram is making a claim about the _number of distinct integration surfaces_. Compressing them collapses the claim.
