...
**Note**: Do NOT manually parse dbt*project.yml or model configs for schema -- always use the script. It handles model config overrides, dbt_project.yml routing rules, PROD* prefix for custom schemas, and defaults to `PROD`.
...
_Make sure MC Bridge is running. Let me know if you want tips on how to install this locally_
```
