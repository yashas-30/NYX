...
const fetchAllUsers = pipe(userIds, TE.traverseArray(fetchUser)); // TaskEither<Error, readonly User[]>
...
// Usage
const fetchUserWithRetry = (userId: string) => retry(fetchUser(userId), 3, 1000);
...
const request = <T>(url: string, options: RequestInit = {}): TE.TaskEither<ApiError, T> =>
  TE.tryCatch(
    async () => {
      const response = await fetch(url, {
        headers: {
...
const wrapPrisma = <T>(operation: () => Promise<T>): TE.TaskEither<DbError, T> =>
...
const userRepository = {
  findById: (id: string): TE.TaskEither<DbError, User> =>
    pipe(
      ...
      TE.chain((user) =>
        user ? TE.right(user) : TE.left({ _tag: 'NotFound', entity: 'User', id })
      )
    ),
...
const createUserService = (input: CreateUserInput) =>
  pipe(
    ...
    TE.chain((existing) =>
      existing ? TE.left({ _tag: 'UniqueViolation' as const, field: 'email' }) : TE.right(undefined)
    ),
    // Create user
...
const readJson = <T>(
  filePath: string
): TE.TaskEither<FileError | { _tag: 'ParseError'; cause: unknown }, T> =>
  pipe(
    readFile(filePath),
    TE.chain((content) =>
      TE.tryCatch(
...
const sendResult = <E, A>(res: Response, te: TE.TaskEither<E, A>, errorStatus: number = 500) =>
  pipe(
    te,
    TE.fold(
