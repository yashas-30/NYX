  ...
  pipe(
    ...
    RTE.flatMap(({ db, logger }) =>
      pipe(
        ...
        RTE.flatMap((user) =>
          user ? RTE.right(user) : RTE.left({ _tag: 'NotFound', resource: 'User', id })
        ),
...
// Create user
export const create = (input: CreateUserInput): RTE.ReaderTaskEither<UserDeps, UserError, User> =>
  pipe(
    RTE.ask<UserDeps>(),
    RTE.flatMap(({ db, hasher }) =>
      pipe(
        ...
        RTE.flatMap(() => RTE.fromTaskEither(hasher.hash(input.password))),
...
// Find by ID
export const findById = (id: string): RTE.ReaderTaskEither<UserDeps, UserError, User> =>
  pipe(
    RTE.ask<UserDeps>(),
    RTE.flatMap(({ db }) =>
      pipe(
        ...
        RTE.flatMap((user) =>
          user ? RTE.right(user) : RTE.left({ _tag: 'UserNotFound' as const, id })
        )
      )
    )
  );
...
const checkEmailUnique = (email: string): RTE.ReaderTaskEither<UserDeps, UserError, void> =>
  pipe(
    RTE.ask<UserDeps>(),
    RTE.flatMap(({ db }) =>
      pipe(
        ...
        RTE.flatMap((existing) =>
          existing ? RTE.left({ _tag: 'EmailExists' as const, email }) : RTE.right(undefined)
        )
      )
    )
  );
...
export const createOrder = (
  ...
  pipe(
    ...
    RTE.bind('products', () =>
      pipe(
        items,
        A.traverse(RTE.ApplicativePar)((item) => ProductService.findById(item.productId)),
        RTE.mapLeft(toOrderError)
      )
    ),
    // Calculate total
    RTE.bind('total', ({ products }) => RTE.right(calculateTotal(products, items))),
    // Process payment
    RTE.bind('payment', ({ user, total }) =>
      pipe(PaymentService.charge(user, total), RTE.mapLeft(toOrderError))
    ),
    // Create order
...
const buildInfrastructure = (config: Config): TE.TaskEither<Error, Infrastructure> =>
  pipe(
    TE.Do,
    TE.bind('db', () =>
      TE.tryCatch(
...
const program: RTE.ReaderTaskEither<AppDeps, AppError, void> = pipe(
  RTE.ask<AppDeps>(),
  RTE.flatMap((deps) => pipe(startServer(deps), RTE.fromTaskEither))
);
...
const wrapPrisma = <A>(operation: () => Promise<A>): TE.TaskEither<DbError, A> =>
...
// Repository factory
export const createRepository = <Model, CreateInput, UpdateInput, WhereUnique, WhereMany>(
  db: PrismaClient,
  delegate: {
  ...
  update: (where: WhereUnique, data: UpdateInput): TE.TaskEither<DbError, Model> =>
...
export const transferFunds = (
  ...
  withTransaction(
    pipe(
      ...
      RTE.bind('transfer', ({ from, to }) => createTransferRecord(from, to, amount)),
...
export const withDeps =
  (deps: AppDeps): MiddlewareHandler =>
  async (c, next) => {
    c.set('deps', deps);
    await next();
  };
...
export const validation = (field: string, message: string, value?: unknown): ValidationError => ({
  _tag: 'ValidationError',
  field,
  message,
  value,
...
export const conflict = (resource: string, field: string, value: string): ConflictError => ({
  _tag: 'ConflictError',
  resource,
  field,
  value,
...
export const toResponseBody = (error: DomainError): { error: string; details?: unknown } => {
  switch (error._tag) {
    case 'NotFoundError':
...
export const withRetry =
  <R, E, A>(maxAttempts: number, baseDelayMs: number, shouldRetry: (error: E) => boolean) =>
  (operation: RTE.ReaderTaskEither<R, E, A>): RTE.ReaderTaskEither<R, E, A> =>
    pipe(
      RTE.ask<R>(),
      RTE.flatMap((deps) => {
...
export const withFallback =
  <R extends { cache: CacheClient }, E, A>(cacheKey: string, ttlSeconds: number) =>
  (operation: RTE.ReaderTaskEither<R, E, A>): RTE.ReaderTaskEither<R, E, A> =>
    pipe(
      RTE.ask<R>(),
      RTE.flatMap(({ cache, ...rest }) =>
        pipe(
          ...
          RTE.tap((result) => RTE.fromTaskEither(cache.set(cacheKey, result, ttlSeconds))),
  ...
  return <R, A>(
    ...
    pipe(
      ...
      RTE.flatMap((deps) => {
        ...
        if (state === 'open' && Date.now() - lastFailure > resetTimeoutMs) {
          state = 'half-open';
        }
