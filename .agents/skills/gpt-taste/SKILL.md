...
- **Hero Layout Options (Randomly Assigned via Python):**
  1. _Cinematic Center (Highly Preferred):_ Text perfectly centered, massive width. Below the text, exactly two high-contrast CTAs. Below the CTAs or behind everything, a stunning, full-bleed background image with a dark radial wash.
  2. _Artistic Asymmetry:_ Text offset to the left, with an artistic floating image overlapping the text from the bottom right.
