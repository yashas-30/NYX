...
| --------------------------------- | ------------------- | --------------------------- |
| `\pvalue{0.023}`                  | _p_ = 0.023         | P-values                    |
| `\psig{< 0.001}`                  | **_p_ = < 0.001**   | Significant p-values (bold) |
| `\CI{0.45}{0.72}`                 | 95% CI [0.45, 0.72] | Confidence intervals        |
| `\effectsize{d}{0.75}`            | d = 0.75            | Effect sizes                |
| `\samplesize{250}`                | _n_ = 250           | Sample sizes                |
...
- Use italics for gene symbols (e.g., _TP53_), regular font for proteins (e.g., p53)
- Follow species-specific gene nomenclature (uppercase for human: _BRCA1_; sentence case for mouse: _Brca1_)
- Specify organism names in full at first mention, then use accepted abbreviations (e.g., _Escherichia coli_, then _E. coli_)
...
- Use binomial nomenclature for species (italicized: _Homo sapiens_)
...
- For broad-impact journals (e.g., _Nature_, _Science_): Define more technical terms, provide context for specialized concepts
