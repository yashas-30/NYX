...
"""Handle /ticket slash command.""" # Acknowledge immediately (within 3 seconds)
ack()
...
def build*task_blocks(task: dict):
...
"value": f"delete*{task['id']}"
},
{
"text": {"type": "plain*text", "text": "Share"},
"value": f"share*{task['id']}"
}
]
}
}
...
if **name** == "**main**": # SocketModeHandler manages the WebSocket connection
