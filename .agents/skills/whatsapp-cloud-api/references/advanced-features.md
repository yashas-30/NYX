...
await sendMultiProduct('5511999999999', 'CATALOG_ID', [
  {
    title: 'Eletronicos',
    product_items: [{ product_retailer_id: 'SKU_001' }, { product_retailer_id: 'SKU_002' }],
  },
  {
    title: 'Acessorios',
    product_items: [{ product_retailer_id: 'SKU_003' }, { product_retailer_id: 'SKU_004' }],
  },
]);
```
...
async function processStatusUpdate(status: WebhookStatus): Promise<void> {
  ...
  await db.messageStatuses.updateOne({ messageId: status.id }, { $set: update });
}
```
