...
export async function adminRouter(fastify: FastifyInstance) {
  ...
  fastify.post('/rules/prune', (request, reply) => {
    try {
      const rules = RulesDb.getRules();
      const maxCount = parseInt(
        String((request.body as any)?.maxCount || process.env.RULES_DB_MAX_ENTRIES || '500'),
        10
      );
      ...
      if (rules.length > maxCount) {
        ...
        fs2.writeFileSync(
          path2.join(CACHE_DIR, 'critic-rules.json'),
          JSON.stringify(rules, null, 2)
        );
      }
      reply.send({ success: true, before, after: rules.length, pruned: before - rules.length });
    } catch (error: any) {
  ...
  fastify.delete('/rules', (_req, reply) => {
    try {
      RulesDb.resetRules();
