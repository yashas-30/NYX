...
import './server/lib/apiAgent.ts'; // ðŸš€ Init global connection pooling
...
async function runDependencyHealthChecks() {
  ...
  try {
    ...
    logger.warn(
      { error: error.message },
      '[DepCheck] Python: NOT FOUND â€” Scrapling service will be unavailable'
    );
  }
  ...
  if (llamaBinaryExists) {
    ...
    logger.warn(
      '[DepCheck] llama-server binary: NOT FOUND â€” Local GGUF models will require download on first use'
    );
  }
  ...
  try {
    ...
    try {
      ...
      logger.warn(
        '[DepCheck] Vulkan driver: NOT DETECTED â€” GPU acceleration may be unavailable for local models'
      );
    }
  }
...
async function startServer() {
  ...
  const antigravityHealthInterval = setInterval(async () => {
    try {
      ...
      logger.warn('[Antigravity] Health check failed â€” restarting Antigravity service...');
      if (antigravityProc) {
        try {
  ...
  server.listen(PORT, '127.0.0.1', () => {
    logger.info(`ðŸš€ NYX READY: http://localhost:${PORT}`);
  });
  ...
  server.on('upgrade', (request, socket, head) => {
    try {
      ...
      if (pathname === '/ws/session-sync') {
        ...
        import('./server/features/local-models/localModelManager.ts')
