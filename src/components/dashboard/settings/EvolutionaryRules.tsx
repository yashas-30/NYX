  ...
  return (
    <div className="mt-6 group p-5 rounded-3xl bg-card border border-white/[0.04] hover:border-[#22D3EE]/25 transition-all duration-300 relative overflow-hidden shadow-lg">
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          <p className="text-[10px] font-black uppercase tracking-[0.25em] text-[#22D3EE]">EVOLVED MEMORY MANAGER</p>
          ...
          evolvedRules.map((rule, idx) => (
            <div key={idx} className="p-3 border border-white/10 rounded-xl bg-white/[0.01] hover:bg-white/[0.03] transition-colors flex flex-col gap-1.5">
              <div className="flex items-center justify-between">
                <span className="text-[10px] px-2 py-0.5 rounded-full bg-[#22D3EE]/10 text-[#22D3EE] border border-[#22D3EE]/20 font-bold uppercase tracking-wider">
                  {rule.metric}
                </span>
              ...
              <div className="text-[11px] font-mono text-[#22D3EE] bg-[#22D3EE]/5 border border-[#22D3EE]/20 rounded-lg p-2 select-all leading-normal">
                {rule.rule}
              </div>
            </div>
          ))
