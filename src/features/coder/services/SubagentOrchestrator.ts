...
Please identify the compile/syntax/import issue, generate the corrected complete file content, and invoke the edit_file tool to fix it.`;
        ...
        return `[VALIDATION ERROR] An error occurred in compile check loop: ${error.message || error}`;
      }
    }
  ...
  private parseToolCalls(text: string): Array<{ tool: string; params: Record<string, any> }> {
    ...
    while ((match = blockRegex.exec(text)) !== null) {
      try {
        addParsed(JSON.parse(match[1].trim()));
        foundInBlocks = true;
      } catch {}
    }
    if (foundInBlocks && calls.length > 0) return calls;

    // 2. Try parsing the entire text directly
    try {
      addParsed(JSON.parse(text.trim()));
      if (calls.length > 0) return calls;
    } catch {}

    // 3. Fallback: extract balanced JSON objects/arrays from mixed text
    let depth = 0;
    let startIndex = -1;
    let inString = false;
    let escape = false;
