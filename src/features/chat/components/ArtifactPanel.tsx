      ...
      <div className="flex items-center justify-between p-4 border-b border-[rgba(255,255,255,0.06)] bg-[#0e1416]">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <span className="text-[11px] font-mono text-[#4A5059]">{artifact.type} {artifact.language ? `• ${artifact.language}` : ''}</span>
          </div>
        </div>
        
        <div className="flex items-center gap-2">
          <button onClick={handleCopy} className="p-1.5 text-[#4A5059] hover:text-[#F8FAFC] hover:bg-[#18181B] rounded transition-colors" title="Copy Content">
            <Copy className="w-4 h-4" />
          </button>
          <button onClick={handleDownload} className="p-1.5 text-[#4A5059] hover:text-[#F8FAFC] hover:bg-[#18181B] rounded transition-colors" title="Download">
            <Download className="w-4 h-4" />
          </button>
          <button onClick={() => setIsFullscreen(!isFullscreen)} className="p-1.5 text-[#4A5059] hover:text-[#F8FAFC] hover:bg-[#18181B] rounded transition-colors" title="Toggle Fullscreen">
            {isFullscreen ? <Minimize2 className="w-4 h-4" /> : <Maximize2 className="w-4 h-4" />}
          </button>
          <div className="w-px h-4 bg-[rgba(255,255,255,0.1)] mx-1" />
          <button onClick={onClose} className="p-1.5 text-[#4A5059] hover:text-[#ffb4ab] hover:bg-[#93000a]/20 rounded transition-colors" title="Close Panel">
            <X className="w-4 h-4" />
          </button>
        </div>
      </div>
