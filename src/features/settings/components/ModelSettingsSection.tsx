...
export const ModelSettingsSection: React.FC<ModelSettingsSectionProps> = ({
  ...
  return (
    <div className="mt-6 group p-5 rounded-3xl bg-card border border-white/[0.04] hover:border-[#22D3EE]/25 transition-all duration-300 relative overflow-hidden shadow-lg">
      <div className="absolute top-0 left-0 w-full h-[2px] bg-gradient-to-r from-[#22D3EE]/50 via-[#22D3EE]/30 to-[#22D3EE]/50 opacity-70 group-hover:opacity-100 transition-opacity" />
      
      <div className="flex items-center justify-between mb-4">
        <div>
          <p className="text-[10px] font-black uppercase tracking-[0.25em] text-[#22D3EE]">LOCAL INFERENCE ENGINE</p>
          <h3 className="text-xs font-bold text-foreground mt-0.5">Quantization Quality / Speed</h3>
        </div>
        <span className={`text-[10px] font-bold uppercase tracking-widest px-2 py-0.5 rounded-full border ${quantSaving ? 'text-[#22D3EE]/80 bg-[#22D3EE]/5 border-[#22D3EE]/15' : 'text-[#22D3EE] bg-[#22D3EE]/10 border-[#22D3EE]/20'}`}>
          ...
          return (
            <button
              ...
              className={`relative p-3 rounded-2xl border text-left transition-all duration-200 cursor-pointer ${
                isSelected
                  ? 'bg-[#22D3EE]/10 border-[#22D3EE]/40 shadow-md shadow-[#22D3EE]/5'
            ...
            >
              ...
              <div className={`text-[10px] font-black uppercase tracking-wider mb-1 ${isSelected ? 'text-[#22D3EE]' : 'text-muted-foreground/80'}`}>
                {tier.label}
              </div>
              ...
              <div className="space-y-1">
                ...
                <div className="flex justify-between text-[9px] font-bold uppercase tracking-wider">
                  ...
                  <span className={isSelected ? 'text-[#22D3EE]' : 'text-muted-foreground/80'}>{tier.vram}</span>
                </div>
              </div>
              <span className={`mt-2 inline-block text-[9px] font-bold uppercase tracking-widest px-1.5 py-0.5 rounded-full ${
                isSelected ? 'bg-[#22D3EE]/20 text-[#22D3EE] border border-[#22D3EE]/30' : 'bg-white/5 text-muted-foreground/60 border border-white/10'
              }`}>
                {tier.badge}
              </span>
            </button>
      ...
      {activeQuantInfo?.warn && (
        <div className="mb-3 px-3 py-2 rounded-xl bg-[#22D3EE]/5 border border-[#22D3EE]/20 text-[10px] text-[#22D3EE]/90 flex items-center gap-2">
