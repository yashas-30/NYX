  ...
  return (
    <div className="mt-6 group p-5 rounded-3xl bg-card border border-white/[0.04] hover:border-[#FF3366]/25 transition-all duration-300 relative overflow-hidden shadow-lg">
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          <p className="text-[10px] font-black uppercase tracking-[0.25em] text-[#FF3366]">LOCAL INFERENCE ENGINE</p>
          <h3 className="text-xs font-bold text-foreground mt-0.5">Quantization Quality / Speed</h3>
        </div>
        <span className={`text-[10px] font-bold uppercase tracking-widest px-2 py-0.5 rounded-full border ${quantSaving ? 'text-[#FF3366]/80 bg-[#FF3366]/5 border-[#FF3366]/15' : 'text-[#FF3366] bg-[#FF3366]/10 border-[#FF3366]/20'}`}>
        ...
        {QUANT_TIERS.map((tier) => {
          ...
          return (
            ...
            >
              ...
              <div className={`text-[10px] font-black uppercase tracking-wider mb-1 ${isSelected ? 'text-[#FF3366]' : 'text-muted-foreground/80'}`}>
                {tier.label}
              </div>
              ...
              <div className="space-y-1">
                ...
                <div className="flex justify-between text-[9px] font-bold uppercase tracking-wider">
                  ...
                  <span className={isSelected ? 'text-[#FF3366]' : 'text-muted-foreground/80'}>{tier.vram}</span>
                </div>
              </div>
              <span className={`mt-2 inline-block text-[9px] font-bold uppercase tracking-widest px-1.5 py-0.5 rounded-full ${
                isSelected ? 'bg-[#FF3366]/20 text-[#FF3366] border border-[#FF3366]/30' : 'bg-white/5 text-muted-foreground/60 border border-white/10'
              }`}>
                {tier.badge}
              </span>
            </button>
