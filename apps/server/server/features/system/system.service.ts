...
export class SystemService {
  ...
  async getHealth() {
    ...
    try {
      const disk = await (checkDiskSpace as any)(APP_STATE_DIR);
    ...
    try {
      ...
      const hasGemini = !!(keys.gemini && keys.gemini.trim().length > 0) || !!(env.GEMINI_API_KEY && env.GEMINI_API_KEY.trim().length > 0);
