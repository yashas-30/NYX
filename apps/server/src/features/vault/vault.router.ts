...
export const vaultRouter: FastifyPluginAsync = async (app: FastifyInstance) => {

{
// Wrapping block to avoid scope issues, typically you can remove the wrapper entirely
const router = vaultRouter;
  app.post('/store', { preHandler: [validate(vaultStoreSchema)] },
      ...
      try {
        if (provider === 'gemini') {
          ...
          if (response.ok) {
            ...
            return reply.code(400).send({ valid: false, error: errData.error?.message || 'Invalid API Key' });
          }
        }
        ...
        return reply.code(400).send({ error: `Validation not supported for provider: ${provider}` });
      } catch (error: any) {
