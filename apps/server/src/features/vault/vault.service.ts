...
function getMasterKey(): Buffer {
  ...
  if (!masterKey) {
    if (env.NODE_ENV === 'production') {
      logger.fatal('[KeyVault] NYX_MASTER_KEY environment variable is not provided. FAILED TO START SERVER.');
      throw new Error('NYX_MASTER_KEY environment variable must be provided in production environment.');
    }
...
export function refreshSessionToken(oldToken: string | undefined): string | null {
  if (!verifySessionToken(oldToken)) return null;
  return createSessionToken(false);
}
