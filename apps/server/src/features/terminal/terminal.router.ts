...
export const terminalRouter: FastifyPluginAsync = async (app: FastifyInstance) => {

{
// Wrapping block to avoid scope issues, typically you can remove the wrapper entirely
const router = terminalRouter;
  app.post('/run', { preHandler: [validate(terminalRunSchema)] },
  ...
  app.post('/prompt', { preHandler: [validate(terminalPromptSchema)] },
  ...
  app.get('/poll', (request: FastifyRequest, reply: FastifyReply) => {
