...
export const adminRouter: FastifyPluginAsync = async (app: FastifyInstance) => {

{
// Wrapping block to avoid scope issues, typically you can remove the wrapper entirely
const router = adminRouter;
  app.get('/logs', (request: FastifyRequest, reply: FastifyReply) => {
  ...
  app.get('/scrapling-status', (_req, reply) => {
    reply.send({ status: scraplingHealthState });
  });

  // MISSING-8: Manual rule prune endpoint
  app.post('/rules/prune', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
  ...
  app.delete('/rules', async (_req, reply) => {
    try {
  ...
  app.get('/debug', (request: FastifyRequest, reply: FastifyReply) => {
    const adminKey = env.ADMIN_KEY as string | undefined;
    if (adminKey) {
      const clientKey = (request.headers['x-admin-key'] || (request.query as any).adminKey) as
        | string
