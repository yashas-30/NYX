...
const INJECTION_PATTERNS: Array<{ pattern: RegExp; label: string }> = [
  ...
  { pattern: /forget\s+(everything|all|your|previous|prior)/i, label: 'forget_instructions' },
  {
    ...
    label: 'system_token_injection',
  },
  {
    pattern: /act\s+as\s+(an?\s+)?(DAN|jailbroken|unrestricted|evil|harmful)/i,
    label: 'jailbreak_persona',
  },
  {
    pattern: /you\s+are\s+now\s+(DAN|uncensored|jailbroken|unrestricted)/i,
    label: 'jailbreak_persona',
  },
  {
    pattern: /pretend\s+(you\s+are|to\s+be)\s+(an?\s+)?(evil|harmful|unrestricted)/i,
    label: 'jailbreak_persona',
  },
  {
    pattern: /do\s+anything\s+now|DAN\s+mode|developer\s+mode\s+enabled/i,
    label: 'jailbreak_mode',
  },
  {
    pattern: /bypass\s+(your\s+)?(safety|filter|restriction|guidelines|guardrail)/i,
    label: 'bypass_safety',
  },
  {
    pattern: /override\s+(your\s+)?(instructions|rules|system\s+prompt)/i,
    label: 'override_instructions',
  },
  {
    pattern: /reveal\s+(your\s+)?(system\s+prompt|instructions|api\s+key|secret)/i,
    label: 'data_exfil',
  },
  {
    pattern: /print\s+(the\s+)?(above|previous|your\s+system)\s+(prompt|instructions)/i,
    label: 'data_exfil',
  },
  {
    pattern: /what\s+(are|is)\s+your\s+(system\s+prompt|hidden\s+instruction)/i,
    label: 'data_exfil',
  },
...
const TOXICITY_PATTERNS: Array<{ pattern: RegExp; label: string }> = [
  {
    pattern:
      /\b(how\s+to\s+(make|build|create|synthesize)\s+(a\s+)?(bomb|explosive|weapon|poison|nerve\s+agent))\b/i,
    label: 'harmful_instructions',
  },
  {
    pattern: /\b(step[s\s]+to\s+(hack|exploit|attack|pwn)\s+(?!my|our|your\s+own))/i,
    label: 'harmful_instructions',
  },
];
...
export function checkInput(
  ...
  if (cfg.allowedIntents.length > 0) {
    ...
    const hasCodeContext =
      /\b(code|function|bug|error|file|script|api|database|react|typescript|rust|python)\b/i.test(
        prompt
      );
    const isMedicalLegal =
      /\b(diagnose|prescription|lawsuit|legal\s+advice|medical\s+advice|sue\s+someone)\b/i.test(
        prompt
      );
    ...
    if (isMedicalLegal && !hasCodeContext && !cfg.allowedIntents.includes('general_chat')) {
      violations.push({
        ...
        detail:
          'Request appears to seek professional medical/legal advice — redirecting to professionals',
      });
