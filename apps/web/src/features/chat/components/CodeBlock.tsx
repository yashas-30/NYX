...
import { isSlidevContent } from '@src/features/artifacts/utils/slidevParser';
import { PresentationDeck } from '@src/features/presentation/components/PresentationDeck';
import { toast } from '@src/shared/components/ui/sonner';
import { buildLivePreviewSrcDoc } from '@src/shared/utils/livePreviewRunner';
import { extractArtifactTitle } from './ChatMessageList';

const PythonSandbox = React.lazy(() =>
  import('./PythonSandbox').then((m) => ({
    default: m.PythonSandbox,
  }))
);
...
export function CodeBlock({
  ...
  useEffect(() => {
    ...
    const now = Date.now();
    if (now - lastCodeUpdateRef.current >= 300) {
      lastCodeUpdateRef.current = now;
      setDebouncedCode(code);
    } else {
      const remaining = 300 - (now - lastCodeUpdateRef.current);
  ...
  // Active tab state: default to 'code' ALWAYS so codeblocks never auto-execute or mount heavy runtimes on boot.
  const [activeTab, setActiveTab] = useState<'split' | 'preview' | 'code' | 'console'>('code');
  ...
  // Mermaid render effect — renders live using debouncedCode ONLY when preview tab is active
  useEffect(() => {
    let isCancelled = false;

    if (isMermaid && (activeTab === 'preview' || activeTab === 'split')) {
  ...
  }, [debouncedCode, isMermaid, activeTab]);

  // HTML / JS Iframe generation with full libraries & storage polyfill (computed only when previewing)
  ...
  }, [debouncedCode, cleanLang, isHtml, isDiagramHtml, isJsScript, activeTab]);
  ...
  const handleOpenInCanvas = () => {
    if (onArtifactClick) {
      onArtifactClick({
        ...
        title: filename || extractArtifactTitle(code, cleanLang),
        content: code,
        language: cleanLang,
      });
    } else {
        ...
        <div className="flex items-center justify-between px-4 py-3 bg-muted/40 border-b border-border gap-3">
          <div className="flex items-center gap-2.5 min-w-0">
            <span className="font-mono text-xs font-bold uppercase tracking-wider text-foreground truncate">
              {filename || extractArtifactTitle(code, cleanLang)}
            </span>
    ...
    if (isReact) {
      return (
        <div className={`w-full ${heightClass} overflow-hidden bg-background`}>
          <React.Suspense
            fallback={
              <div className="p-4 text-xs font-mono text-zinc-400">Loading React Sandbox...</div>
            }
          >
            <Sandpack
              template="react-ts"
    ...
    if (isPython) {
      return (
        <div className="w-full h-[360px] overflow-hidden bg-background">
          <React.Suspense
            fallback={
              <div className="p-4 text-xs font-mono text-zinc-400">Loading Python Sandbox...</div>
            }
          >
