    ...
    <div className="mt-6 group p-5 rounded-3xl bg-card border border-border hover:border-accent/25 transition-all duration-300 relative overflow-hidden shadow-lg">
      ...
      <div className="space-y-3 max-h-60 overflow-y-auto custom-scrollbar pr-1 mb-4">
        {evolvedRules.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-6 text-center border border-dashed border-border rounded-xl bg-secondary/10">
          ...
          evolvedRules.map((rule, idx) => (
            <div
              key={idx}
              className="p-3 border border-border rounded-xl bg-secondary/20 hover:bg-secondary/40 transition-colors flex flex-col gap-1.5"
            >
              <div className="flex items-center justify-between">
                <span className="text-[10px] px-2 py-0.5 rounded-full bg-accent/10 text-accent border border-accent/20 font-bold uppercase tracking-wider">
                  {rule.metric}
                </span>
              ...
              <div className="text-[11px] font-mono text-accent bg-accent/5 border border-accent/20 rounded-lg p-2 select-all leading-normal">
                {rule.rule}
              </div>
            </div>
          ))
