  ...
  persist(
    (set, get) => ({
      ...
      checkLimit: (
        ...
        if (limits.tpm !== null && limits.tpm !== undefined && currentUsage.tpmUsed >= limits.tpm)
          return 'tpm';
          ...
          return {
            usage: {
              ...
              [key]: {
                ...currentUsage,
                rpmUsed:
                  reason === 'rpm' && limits.rpm
                    ? Math.max(currentUsage.rpmUsed, limits.rpm)
                    : currentUsage.rpmUsed,
                tpmUsed:
                  reason === 'tpm' && limits.tpm
                    ? Math.max(currentUsage.tpmUsed, limits.tpm)
                    : currentUsage.tpmUsed,
                rpdUsed:
                  reason === 'rpd' && limits.rpd
                    ? Math.max(currentUsage.rpdUsed, limits.rpd)
                    : currentUsage.rpdUsed,
              },
            },
          };
        });
