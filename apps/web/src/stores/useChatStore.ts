...
function deriveTitleFromMessages(messages: ChatMessage[]): string {
  ...
  text = text.replace(
    /^(?:hello|hi|hey|greetings|good\s+(?:morning|afternoon|evening)|yo|sup)\b[\s,!.:\-]*/i,
    ''
  );
  text = text.replace(
    /^(?:can\s+you\s+)?(?:please\s+)?(?:search\s+(?:the\s+web\s+for|for|online\s+for)?|tell\s+me\s+about|find\s+(?:out\s+)?|look\s+up|explain|describe|research)\s*/i,
    ''
  );
...
function detectTopicCategory(messages: ChatMessage[]): string {
  ...
  if (
    /\b(?:search|web|deep|research|news|price|prices|paper|arxiv|github|lookup|find|google)\b/i.test(
      text
    )
  ) {
    return 'Research';
  ...
  if (
    /\b(?:code|function|class|bug|error|refactor|debug|rust|python|typescript|react|api|sql|component)\b/i.test(
      text
    )
  ) {
    return 'Code & Dev';
  ...
  return {
    ...
    updateSession: (sid, messages) => {
      ...
      if (latestUpdated && !(latestUpdated as ChatSession).folderId && messages.length > 0) {
        const dateStr = new Date((latestUpdated as ChatSession).createdAt)
          .toISOString()
          .slice(0, 7);
        const category = detectTopicCategory(messages);
        const folderName = `${dateStr} — ${category}`;
