...
components: # inside the cluster, plus bars and cross-cutting rows
  ...
  - {
      ...
      kind: cross-cutting,
      icon: keycloak,
      subtitle: 'Keycloak · LDAP · OIDC',
      color: '#b85450',
    }
  - {
      name: 'Monitoring',
      chevron: 'Observability',
      kind: cross-cutting,
      icon: prometheus,
      subtitle: 'Prometheus · Grafana · Loki',
    }
...
- **Count constraint:** `len(verticals) == len(bars) + len(crosscuts)`. The right strip is split evenly among all verticals (§2.9), so visual alignment between a chevron and its bar/row is approximate — the _label_ is what matters, not the y-pixel match.
...
- **No edges:** vertical chevrons emit no connectors themselves. They are _labels for a column of cross-cutting concern_.
...
- More than one focal node — MinIO/S3 (or whichever storage hub) is _the_ focal point.
