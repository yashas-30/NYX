...
**Best for:** what a person does across the stages of an experience and how it _feels_ at each one. The sentiment curve is the load-bearing element — without it this is just a process diagram with extra rows, so if you can't name a sentiment for every stage, use **Process** or **Timeline** instead.
...
- `assets/example-journey.html` — minimal light. _Trial to paid: the first week_, 5 stages, trough at "Hit the limit".
