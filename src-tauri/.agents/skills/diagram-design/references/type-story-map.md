...
- **Kanban** shows _state_ — columns are Todo/Doing/Done, there is no narrative order and nothing to slice.
- **User journey** shows _one persona's feelings_ across stages via a sentiment curve.
...
2. **Walking skeleton row** — directly under each activity, its user _steps_ as smaller cards (`rx=4`, 32px tall, white fill, `ink` stroke), Geist sans 12px. One or two per activity; reserve room for two so every activity's row bottom aligns even when it only has one step.
...
- `assets/example-story-map.html` — minimal light. _Reporting, first release_: four activities (`Find the data`, `Build the report`, `Share it`, `Trust it`), MVP/Release 2/Later slices, release cut under MVP, `Row-level permissions` flagged as the riskiest story.
