          ...
          "rows": [
            {
              ...
              "description": "Com arroz e salada - R$32"
            },
          ...
          "rows": [
            {
              "id": "bebida_1",
              "title": "Suco Natural",
              "description": "Laranja, limao ou maracuja - R$8"
            },
  ...
  "contacts": [
    {
      ...
      "phones": [{ "phone": "+5511988887777", "type": "WORK", "wa_id": "5511988887777" }],
      "emails": [{ "email": "suporte@techco.com.br", "type": "WORK" }],
      "org": {
        "company": "TechCo Solucoes"
      },
      "urls": [{ "url": "https://techco.com.br", "type": "WORK" }]
    }
  ]
}
```
...
async function sendContactMessage(to: string, contacts: ContactInfo[]): Promise<string> {
  const url = `https://graph.facebook.com/v21.0/${process.env.PHONE_NUMBER_ID}/messages`;
...
async function sendReaction(to: string, messageId: string, emoji: string): Promise<string> {
  const url = `https://graph.facebook.com/v21.0/${process.env.PHONE_NUMBER_ID}/messages`;
...
async function sendWhatsAppRequest<T>(payload: T): Promise<Record<string, unknown>> {
  ...
  try {
    ...
    if (err instanceof AxiosError && err.response) {
      ...
      throw new Error(`WhatsApp API Error [${waError.error.code}]: ${waError.error.message}`);
    }
    throw err;
  }
}
