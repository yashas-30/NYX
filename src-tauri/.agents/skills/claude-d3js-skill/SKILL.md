...
function drawHeatmap(data) {
  ...
  const legend = svg.append('g').attr('transform', `translate(${width - 60},${margin.top})`);
...
// Chained transitions
circles.transition().duration(500).attr('fill', 'orange').transition().duration(500).attr('r', 15);
...
const bandScale = d3.scaleBand().domain(['A', 'B', 'C', 'D']).range([0, 400]).padding(0.1);
...
svg.selectAll('text').style('font-family', 'Inter, sans-serif').style('font-size', '12px');

// Use subtle grid lines
g.selectAll('.tick line').attr('stroke', colours.gridLines).attr('stroke-dasharray', '2,2');
```
