...
```javascript
const quantizeScale = d3.scaleQuantize().domain([0, 100]).range(['low', 'medium', 'high']);
...
```javascript
const divergingScale = d3.scaleDiverging(d3.interpolateRdBu).domain([-10, 0, 10]);
...
```javascript
const pointScale = d3.scalePoint().domain(['A', 'B', 'C', 'D']).range([0, 400]).padding(0.5);
...
// Time scale ticks
const timeScale = d3.scaleTime().domain([new Date(2020, 0, 1), new Date(2024, 0, 1)]);
