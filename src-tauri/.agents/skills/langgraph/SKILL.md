...
messages: Annotated[list, add_messages] # add_messages reducer appends, doesn't overwrite

# 2. Define Tools

@tool
def search(query: str) -> str:
"""Search the web for information.""" # Implementation here
...
class ResearchState(TypedDict): # Messages append (don't overwrite)
...
def researcher(state: ResearchState) -> dict: # Only return fields being updated
return {
...
def writer(state: ResearchState) -> dict: # Access accumulated state
...
def agent(state: ApprovalState) -> dict: # Agent decides on action
...
action = state["pending_action"] # Execute the approved action
