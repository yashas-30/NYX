...
const fetchUser = (id: string): TE.TaskEither<Error, User> =>
  pipe(
    ...
    TE.flatMap((response) =>
      response.ok ? TE.right(response) : TE.left(new Error(`HTTP error: ${response.status}`))
    ),
    TE.flatMap((response) =>
      TE.tryCatch(
        () => response.json(),
...
const getManagerEmail = (users: User[], employee: { managerId?: string }): O.Option<string> =>
  pipe(
...
// Clean composition
const processFile = (inputPath: string, outputPath: string): TE.TaskEither<Error, void> =>
  pipe(
    readFile(inputPath),
...
const processMultipleFilesParallel = (
  ...
  pipe(
    files,
    A.traverse(TE.ApplicativePar)(({ input, output }) => processFile(input, output))
  );
...
const updateEmail = (userId: string, newEmail: string): RTE.ReaderTaskEither<AppEnv, Error, void> =>
  pipe(
    ...
    RTE.flatMap((user) => {
      ...
      return pipe(
        ...
        RTE.flatMap(() =>
          sendEmail(oldEmail, 'Email Changed', `Your email has been changed to ${newEmail}`)
        ),
...
const productStatsMonoid: M<ProductStats> = {
  ...
  concat: (a, b) => ({
    ...
    categoryCounts: pipe(a.categoryCounts, R.union({ concat: (x, y) => x + y })(b.categoryCounts)),
...
const paginate = <T>(
  ...
  const go = (cursor: string | null, accumulated: T[]): TE.TaskEither<Error, T[]> =>
    pipe(
      fetchPage(cursor),
      TE.flatMap(({ items, nextCursor }) => {
        const newAccumulated = [...accumulated, ...items];
        return nextCursor === null ? TE.right(newAccumulated) : go(nextCursor, newAccumulated);
      })
    );
...
const raceTaskEithers =
  <E, A>(tasks: Array<TE.TaskEither<E, A>>): TE.TaskEither<E, A> =>
  () =>
...
// Chain of fallbacks
const withFallbacks = <E, A>(tasks: Array<TE.TaskEither<E, A>>): TE.TaskEither<E, A> =>
...
const processData = (input: string): TE.TaskEither<Error, Result> =>
  pipe(parseInput(input), TE.fromEither, TE.flatMap(fetchData));
```
...
const result: E.Either<Error, User> = pipe(parseUserId(input), E.flatMap(fetchUser));
```
...
const formatName = (first: string, last: string): string => pipe(first, (f) => `${f} ${last}`);
```
...
const processOrder = (order: Order): TE.TaskEither<Error, Result> =>
  pipe(validateOrder(order), TE.fromEither, TE.flatMap(enrichOrder), TE.flatMap(submitOrder));
