...
const response = await admin.graphql(`     query GetProducts($first: Int!, $after: String) {
...
const response = await admin.graphql(`     mutation UpdateProduct($input: ProductInput!) {
...
const response = await admin.graphql(`     mutation CreateSubscription($name: String!, $lineItems: [AppSubscriptionLineItemInput!]!, $returnUrl: URL!, $test: Boolean) {
...
```typescript
async function shopifyRequest(fn: () => Promise<Response>, maxRetries = 3): Promise<Response> {
  let lastError: Error;
