...
"""
def reflect_and_improve(task, max_iterations=3): # Initial generation
...
def trim_context(messages, max_tokens=4000): # Keep system message and recent messages
system = messages[0]
recent = messages[-10:]
...
state = agent.get_state(config)
if human_approves(state): # Continue from pause point
agent.invoke(None, config)
else: # Modify state and continue
...
def trim_context(messages, max_tokens=4000): # Keep: system prompt + last N messages # Summarize: everything in between
...
def validate_expense(expense): # Cross-check with external sources
...
def **init**(self, max_tokens=100000):
...
@trace
def agent_step(state): # Automatically traced with inputs/outputs
return next_state
