...
function searchKnowledgeBase(topic) {
  ...
  const articles = kbIndex.categories.filter(
    (cat) => cat.tags.includes(topic) || cat.keywords.includes(topic)
  );
...
const articleMapping = {
  ...
  Diabetes: ['chronic-disease/diabetes-management', 'nutrition/diabetic-diet'],
  'Sleep Deprivation': ['sleep/sleep-hygiene', 'sleep/sleep-quality-improvement'],
  'Weight Gain': ['nutrition/healthy-diet', 'nutrition/calorie-management'],
  'High Stress': ['mental-health/stress-management', 'mental-health/mindfulness'],
};
```
