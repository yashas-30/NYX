...
const message =
  user === null ? 'No user' : user.isAdmin ? `Admin: ${user.name}` : `User: ${user.name}`;
...
const ValidationFailed = (errors: string[]) => ({ _tag: 'ValidationFailed' as const, errors });
...
const validateUser = (data: unknown): E.Either<string, User> =>
  pipe(
    ...
    E.bind('age', ({ obj }) =>
      typeof obj.age === 'number' && obj.age >= 0 ? E.right(obj.age) : E.left('Valid age required')
    ),
...
const processOrder = (orderId: string): TE.TaskEither<string, Receipt> =>
  pipe(
    ...
    TE.flatMap((order) => (order ? TE.right(order) : TE.left('Order not found'))),
