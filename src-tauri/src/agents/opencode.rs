...
pub async fn run_opencode_agent(context: StreamContext, messages: Vec<Value>, app: AppHandle, event_name: String) -> Result<String> {
    let _ = app.emit(&event_name, json!({"type": "thinking", "content": "\n━━━ [OpenCode Agent] Analyzing Codebase... ━━━\n"}));
