// ─────────────────────────────────────────────────────────────────────────────
// NYX — Groq Provider Orchestrator (Ultra-Fast LPU Inference)
// ─────────────────────────────────────────────────────────────────────────────
...
static GROQ_CLIENT: LazyLock<Client> = LazyLock::new(|| build_fast_http_client(64, 120));
...
/// Builds request payload, headers, and URL for Groq API
pub fn build_request(req: &UnifiedRequest) -> Result<(String, Value, HeaderMap), String> {
    ...
    if api_key.is_empty() {
        return Err("Groq API key is required. Please set your Groq API key in Settings → API Keys or set the GROQ_API_KEY environment variable (free keys available at console.groq.com/keys).".to_string());
    }
...
/// Parses an SSE JSON chunk from Groq (handles standard text, thinking/reasoning, and tools)
    ...
    if let Some(choices) = val.get("choices").and_then(|c| c.as_array()) {
        if let Some(choice) = choices.first() {
            if let Some(delta) = choice.get("delta") {
                // Support both `reasoning` and `reasoning_content` for DeepSeek/thinking models on Groq
...
/// Executes streaming generation on Groq LPU API with automatic 429 retry and tool fallback
...
/// Helper to cleanly extract error message from Groq JSON error responses
fn extract_groq_error(text: &str) -> Option<String> {
...
/// Checks API key validity for Groq with 60-second caching and 429 tolerance
pub async fn check_quota(api_key: Option<String>) -> Result<QuotaResponse, String> {
    let key = api_key.unwrap_or_default();
    if let Some(err) = validate_key_format("groq", &key) {
        ...
        if let Some((valid, timestamp)) = cache.get(&cache_key) {
            ...
            if timestamp.elapsed() < std::time::Duration::from_secs(60) {
                return Ok(QuotaResponse {
                    ...
                    valid: *valid,
                    provider: "groq".into(),
                    message: if *valid { "Groq API key is active.".into() } else { "Groq API key appears invalid.".into() },
                });
            }
        }
    }
