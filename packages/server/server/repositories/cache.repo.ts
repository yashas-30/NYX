...
export class CacheRepository {
  ...
  public static async get(key: string): Promise<CacheEntry | null> {
    const records = await db.select().from(this.table).where(eq(this.table.key, key)).limit(1);
  ...
  public static async set(
    ...
    const existing = await db.select().from(this.table).where(eq(this.table.key, key)).limit(1);
