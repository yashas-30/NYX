...
import { startRetentionPruner } from './dataRetention.js';
...
export function spawnBackgroundServices() {
  ...
  async function spawnScrapling() {
    try {
      ...
      const pythonPath = findPythonPath();
      const scraplingScriptPath = path.join(
        appsServerDir,
        'server',
        'python',
        'scrapling_server.py'
  ...
  // Start data retention pruner
  const stopRetentionPruner = startRetentionPruner();
  ...
  const scraplingHealthInterval = setInterval(async () => {
    try {
      ...
      logger.warn(
        ...
        '[Scrapling] Health check failed - restarting Scrapling service...'
      );
      if (scraplingProc) {
        try {
  ...
  const antigravityHealthInterval = setInterval(async () => {
    try {
      ...
      logger.warn(
        { error: err?.message || err },
        '[Antigravity] Health check failed — restarting Antigravity service...'
      );
      if (antigravityProc) {
        try {
...
export function registerShutdownHandlers(
  ...
  const shutdown = () => {
    ...
    const closeExpress = expressServer
      ? new Promise<void>((resolve) => expressServer.close(() => resolve()))
      : Promise.resolve();

    Promise.all([closeFastify, closeExpress])
      .then(() => {
        process.exit(0);
      })
      .catch((err) => {
