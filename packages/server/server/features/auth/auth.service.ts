...
export class AuthService {
  ...
  /**
   * Authenticates the user and returns profile data (GDPR Art. 15/20).
   * Conversations and messages are not linked to users via foreign key
   * in the current schema, so only profile-level data is exported here.
   */
  static async exportUserData(username: string, passwordPlain: string) {
    const userRecords = await db.select().from(users).where(eq(users.username, username)).limit(1);
    if (userRecords.length === 0) {
    ...
    const isMatch = await bcrypt.compare(passwordPlain, user.passwordHash);
    if (!isMatch) {
      throw new AppError(401, 'Invalid password', 'AUTH_INVALID_CREDENTIALS');
    }
  ...
  /**
   * Authenticates and permanently deletes the user profile (GDPR Art. 17).
   * Session tokens and the user row are removed.
   */
  static async deleteUser(username: string, passwordPlain: string) {
    const userRecords = await db.select().from(users).where(eq(users.username, username)).limit(1);
    if (userRecords.length === 0) {
      throw new AppError(404, 'User not found', 'AUTH_USER_NOT_FOUND');
    }
    ...
    // Delete user — cascading deletes handle associated data
