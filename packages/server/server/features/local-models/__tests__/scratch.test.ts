...
describe('scratch test', () => {
  it('prints compat and models', async () => {
    ...
    logger.info(
      { allCompatibleModelIds: compatibility.allCompatibleModelIds },
      'ALL COMPATIBLE MODEL IDS'
    );
    logger.info(
      { count: compatibility.presetsCompatibility?.length },
      'PRESETS COMPATIBILITY COUNT'
    );
