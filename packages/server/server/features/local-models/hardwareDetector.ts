...
import { MODELS_DIR as BASE_DIR } from '../../lib/paths.js';
...
> {
  try {
    ...
    const list = graphics && graphics.controllers ? graphics.controllers : [];
    ...
    logger.warn(err, '[GPU Detection] Failed to query systeminformation graphics');
    try {
      const freeVram = await HardwareDetector.getFreeVram();
      if (freeVram > 0) {
        return [
...
export async function calculateOptimalLayers(
  ...
  if (model && model.status === 'completed' && model.filePath) {
    try {
      ...
      logger.warn(err, 'Failed to parse GGUF metadata, falling back to heuristics');
    }
  } else if (model) {
  ...
  const availableVram = primaryGPU.vramBytes;
