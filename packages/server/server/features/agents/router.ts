...
export class SmartRouter {
  ...
  async route(prompt: string, config: RouterConfig, apiKeys: Record<string, string> = {}): Promise<RoutingDecision> {
    ...
    const scored = await Promise.all(
      candidates.map(async (model) => {
        ...
        // Skip if no API key
        if (!key && (model.provider as string) !== 'pollinations' && model.provider !== 'nyx-native') {
          return null;
        }
  ...
  private getCapabilityScore(model: ModelOption, prompt: string): number {
    ...
    if (prompt.length > 10000) {
      const cwStr = model.specs?.contextWindow?.toUpperCase() || '0';
      const num = parseFloat(cwStr.replace(/[^0-9.]/g, '')) || 0;
      let rawWindow = num;
      if (cwStr.includes('M')) rawWindow *= 1000000;
      else if (cwStr.includes('K')) rawWindow *= 1000;
      
      if (rawWindow >= 100000) score += 0.2; // 100K+
    }
