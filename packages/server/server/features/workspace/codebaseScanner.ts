...
export class CodebaseScanner {
  ...
  public static async indexWorkspace(): Promise<void> {
    ...
    return await traceActiveSpan('CodebaseScanner.indexWorkspace', async (span) => {
      const root = getWorkspaceRoot();
      logger.info(`[RAG] Indexing workspace files at: ${root}`);
      const files = this.scanDirectory(root);

      span.setAttribute('workspace.root', root);
      span.setAttribute('workspace.file_count', files.length);
      ...
      span.setAttribute('workspace.indexed_count', indexCount);
  ...
  public static async search(query: string, maxResults = 5): Promise<SearchResult[]> {
    const { traceActiveSpan } = await import('../../lib/otel.js');
    return await traceActiveSpan('CodebaseScanner.search', async (span) => {
      await this.init();
      const root = getWorkspaceRoot();
      logger.info(`[RAG] Searching index in "${root}" semantically for: "${query}"`);

      span.setAttribute('search.query', query);
      span.setAttribute('search.max_results', maxResults);
      ...
      span.setAttribute('search.results_count', topResults.length);
      logger.info(
        {
          matches: topResults.map((r) => `${r.path} (similarity: ${r.relevanceScore.toFixed(2)}%)`),
        },
