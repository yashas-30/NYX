...
import { projectFiles, pgDbProjectFiles } from '../../db/schema.js';
...
export class ChatService {
  async streamChat(
    ...
    if (params.projectId) {
      try {
        const files = !isPg
          ? await db.select().from(projectFiles).where(eq(projectFiles.projectId, params.projectId))
          : await db.select().from(pgDbProjectFiles).where(eq(pgDbProjectFiles.projectId, params.projectId));
        ...
        logger.warn(`[ChatService] Failed to fetch project context: ${err.message}`);
      }
    }
    ...
    finalPrompt += webContext + projectContext;
