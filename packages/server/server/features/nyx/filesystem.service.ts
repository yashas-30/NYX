...
import { userPreferences, pendingFileWrites } from '../../db/schema.js';
...
export class FilesystemService {
  private getNyxIgnorePatterns(): string[] {
    ...
    if (fs.existsSync(ignorePath)) {
      try {
        ...
        logger.error({ err }, '[Filesystem] Failed to read .nyxignore');
      }
    }
    return patterns;
  }
  ...
  async writeFile(filePath: string, content: string, overwrite?: boolean, agentRunId?: string) {
    ...
    try {
      ...
      logger.error({ err }, '[Filesystem] Failed to query user preferences for auto-approve');
    }
