...
export class KeyManagerService {
  ...
  private refreshKeys(provider: string): string[] {
    ...
    const envKeysStr = (env as any)[envVar] || (env as any)[`${provider.toUpperCase()}_API_KEY`];
    let keys: string[] = [];

    if (envKeysStr) {
      keys = (envKeysStr as string)
        .split(',')
        .map((k: string) => k.trim())
        .filter(Boolean);
    }
    ...
    try {
      ...
      if (vaultKeyStr) {
        const vKeys = (vaultKeyStr as string)
          .split(',')
          .map((k: string) => k.trim())
          .filter(Boolean);
        keys = [...keys, ...vKeys];
  ...
  getNextKey(provider: string): string {
    ...
    const now = Date.now();
    const currentIndex = this.providerIndices.get(provider) || 0;
