...
export const sessions = sqliteTable('sessions', {
  ...
  createdAt: integer('created_at').notNull(),
  revokedAt: integer('revoked_at'), // null until explicitly revoked via logout
});
...
export const dbMessages = sqliteTable('db_messages', {
  ...
  sessionId: text('session_id')
    .notNull()
    .references(() => dbSessions.id, { onDelete: 'cascade' }),
  role: text('role').notNull(), // 'user' | 'assistant' | 'system'
  content: text('content').notNull(),
...
export const pgDbMessages = pgTable('db_messages', {
  ...
  sessionId: pgText('session_id')
    .notNull()
    .references(() => pgDbSessions.id, { onDelete: 'cascade' }),
...
// --- PROJECT SYSTEM (Postgres) ---
export const pgDbProjects = pgTable('db_projects', {
  id: pgText('id').primaryKey(),
  name: pgText('name').notNull(),
...
export const pgDbProjectFiles = pgTable('db_project_files', {
  id: pgText('id').primaryKey(),
  projectId: pgText('project_id')
    .notNull()
    .references(() => pgDbProjects.id, { onDelete: 'cascade' }),
  ...
  uploadedAt: pgTimestamp('uploaded_at').defaultNow(),
  extractedText: pgText('extracted_text'),
});

export const pgDbProjectSessions = pgTable('db_project_sessions', {
  id: pgText('id').primaryKey(),
  projectId: pgText('project_id')
    .notNull()
    .references(() => pgDbProjects.id, { onDelete: 'cascade' }),
